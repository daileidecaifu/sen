...
import sen.wedding.com.weddingsen.databinding.SecondSaleContractBinding;
...
public class SecondSaleContractActivity extends BaseActivity implements View.OnClickListener, RequestHandler<ApiRequest, ApiResponse> {

    SecondSaleContractBinding binding;
...
//        });
        ...
        //签单时间
        binding.llSignUpTime.tvItemSelectTitle.setText(getString(R.string.pay_time));
    ...
    private void submitertificate(String imageUrls) {
        if (orderId != -1) {
            submitCertificateRequest = new ApiRequest(URLCollection.URL_DOMAIN + URLCollection.URL_SECOND_SALE_SIGN, HttpMethod.POST);
            HashMap<String, String> param = new HashMap<>();
            param.put("access_token", BasePreference.getToken());
